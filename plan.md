...
> - 🔄 **Phase 5 (scaffolded)** — `apps/mobile` Expo (Expo Router) app consuming the SAME shared packages: root layout mounts `@livedrop/realtime`'s `SocketProvider`, live-feed reads `/api/streams`, watch screen drives chat/reactions via `@livedrop/features` `useChat`. Proves "share logic, not pixels". Monorepo-aware Metro + single-React config. **Not yet run on a device/simulator** (needs `expo start`); web pinned to `@types/react` 19 to avoid RN's 18 types bleeding across the workspace.
