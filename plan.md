...
> - 🔄 **Phase 4 (in progress)** — order domain in core (schemas + `pending→…→delivered` state machine + paise money helpers); server Order model + REST (`POST /api/orders` idempotent, `:id/pay`, `:id`) with **Razorpay-when-configured + mock fallback**; `useCheckout` in features; web "Pay Now" now persists a real order. Data store stays Mongoose for now — **Postgres/Prisma migration deferred until DB infra is provisioned**. Next: inventory/reservations, delivery tracking, Razorpay webhook capture.
> - 🔄 **Phase 5 (feature parity, typecheck-verified)** — `apps/mobile` Expo (Expo Router) app at near-parity with web, all on the SAME shared packages: live feed, **watch** (chat, reactions, polls, coupon banner, featured-product → cart, **cart sheet + checkout**, **order tracker**), and a **host Go Live** screen (feature products, flash coupons, launch polls). Shared `CartProvider`/`useCart`, `useFeaturedProduct`, `useHostControls`, and a shared demo catalog were added to make this "screens only, no new logic". Whole workspace aligned on **React 19** so mobile typechecks against the shared components. **Blocked (native deps):** live video playback (LiveKit RN SDK / react-native-webrtc) and host camera (`expo-camera`) — shown as placeholders. Not yet run on a device (`expo start`).
> - 🔄 **Phase 6 (in progress)** — delivery + inventory domains in core (delivery `assigned→…→delivered` state machine, hyperlocal `InventoryItem`/`canFulfill`); server Delivery model + **deterministic simulated-rider** tracking endpoint (auto-created when an order is paid); `useDeliveryTracking` in features; web `OrderTracker` (live status + ETA) shows after checkout. The "10-minute" promise made visible without a real fleet. Next: real rider GPS + map, Redis inventory reservation, dark-store geo-routing.
> - ✅ **Phase 7 (testing + CI)** — Vitest unit tests for the pure core logic (money, order/delivery state machines, inventory) — **18 tests green**; `test` task wired into Turborepo; GitHub Actions CI (`.github/workflows/ci.yml`) runs install → typecheck → test → build on every push/PR.
> - 🔄 **Phase 8 (payment hardening + coverage)** — Razorpay **webhook capture** route (HMAC signature verify against raw body → order paid → delivery assigned; 501 until secret set); `orderTotalPaise` helper in core; broadened tests to **24 green** (order totals + domain schema parsing). Razorpay real path still unverified end-to-end (needs keys).
